...
// Dark mode: always use the inverted (white) logo
const LOGO_B64 = Buffer.from(fs.readFileSync(path.join(BASE, 'assets/y1-logo-invers.svg'), 'utf8')).toString('base64');

// Peace-hand icon recolored to white
...
const BG  = '#212121';
const FG  = '#ffffff';
const ACC  = config.colors.accent;   // #ff78ba — unchanged
...
function svgIllustration(slide) {
  ...
  if (slide.isHook) {
    ...
    // 320×320 max keeps the illustration below the content box on content slides
...
// Y1 style guide: opacity 0.7 for secondary text
// Word-wraps at W - PAD*2 to keep equal left/right margins
...
// dotSize defaults to 6; pass 12 for the larger step-label variant
function svgPixelHeadline(text, x, y, fontSize = 12, dotSize = 6) {
  const gap = 8;
  // Center dot vertically with cap height: capCenter = y - (capHeight/2), capHeight ≈ fontSize * 0.7
...
// Light surface box on dark background, no border
function svgContentBlock(terminalHtml, x, y, w, activateLabel) {
  ...
  // Light surface on dark slide — #f1f3f4 fill, no border
  out += `<rect x="${x}" y="${y}" width="${w}" height="${boxH}" fill="#f1f3f4"/>\n`;
  ...
  allRows.forEach((row, i) => {
    ...
    const tspans = parts.map(part => {
      // Text inside light box must be dark
      let fill = '#000000', fw = 300, ff = 'TWKEverett', fs = 32, op = 1;
      switch (part.type) {
...
function ctaSlide(slide) {
  ...
  const eyebrowY    = midY - 168;       // 522: 28px pixel headline, same as content eyebrow
  const kBoxY       = midY - 116;       // 574: keyword box
  const kTextY      = midY - 48;        // 642: keyword text baseline
  const followY     = midY + 46;        // 736: follow line, 30% bigger → 20px
  const claimY      = followY + 52;     // 788: brand claim
  const peaceIconSz = 48;
  const peaceIconCY = 1080;             // peace-hand icon center Y (higher position)
  const peaceIconX  = Math.round((W - peaceIconSz) / 2);
  const signoffY    = peaceIconCY + peaceIconSz / 2 + 44;  // 1148: Bundesgarten Ciao

  const body = `
<!-- Eyebrow — 28px pixel headline, same size as content slide eyebrow -->
${svgPixelHeadlineCentered('DAS WICHTIGSTE', eyebrowY, 28, 12)}

<!-- Keyword box (TWKEverettMono, uppercase) -->
...
<!-- Bundesgarten Ciao — 30% bigger, full white -->
...
if (require.main === module) {
  ...
  const outputDir = path.join(BASE, 'output-dark');
